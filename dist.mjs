...
const defaultBuildCfg = {
  manifestProps: ["name", "version", "main", "domain"],
...
const manifestProps =
  packageManifest.build?.manifestProps || defaultBuildCfg.manifestProps;
const prodManifest = {};
...
try {
  await writeFile(
    path.resolve(appBaseDir, "package.json"),
    JSON.stringify(prodManifest, null, 4)
  );
} catch (error) {
...
for (const osType of appOsTypes) {
  ...
  const nwVersion =
    packageManifest.devDependencies.nw.split("-")[0] ||
    defaultBuildCfg.nwVersion;
